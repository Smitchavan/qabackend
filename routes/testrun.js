...
router.post("/", async (req, res) => {
  try {
    ...
    let testcases = req.body[0].data.testcases;
    ...
    if (existingTestSet) {
      return res
        .status(400)
        .json({ message: `Test set '${testsetname}' already exists` });
    }
...
router.post("/updatesteps", async (req, res) => {
  try {
    let { _id, testcases, runStarttime, runEndtime, ...rest } =
      req.body.data.data;
    ...
    const actualDuration = moment.duration(
      totalDuration - pauseDuration + resumeDuration
    );
    ...
    const totalDurationn = updatedtestcases.reduce(
    ...
    const runtotal =
      moment.duration(moment(runend).diff(moment(runstart))).asMilliseconds() /
      60000;
